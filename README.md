...
- **Email** — the footer has a subscribe form for a self-hosted [listmonk](https://listmonk.app) instance; it stays hidden until `LISTMONK_URL` and `LISTMONK_LIST_UUID` are set in `src/config.ts`. Recommended setup: listmonk + Postgres via Docker Compose on the VPS behind TLS, with actual delivery relayed through Amazon SES (or similar) — never send from the VPS IP directly. Enable listmonk's public subscription page so the form's plain POST to `/subscription/form` works. listmonk has no built-in RSS-to-email; a small scheduled job calling its campaign API with new feed items covers that. The privacy policy's "Email updates" section describes this self-hosted setup — revise it if you switch providers.
