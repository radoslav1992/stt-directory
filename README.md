...
- **Email** — the footer has a subscribe form for a self-hosted [listmonk](https://listmonk.app) instance; it stays hidden until `LISTMONK_URL` and `LISTMONK_LIST_UUID` are set in `src/config.ts`. Setup: listmonk + Postgres via Docker Compose on the VPS behind TLS, with delivery relayed through [Cloudflare Email Sending](https://developers.cloudflare.com/email-service/) — never send from the VPS IP directly. In listmonk's SMTP settings: host `smtp.mx.cloudflare.net`, port `465`, TLS `SSL/TLS`, username the literal string `api_token`, password a Cloudflare API token scoped to only "Email Sending: Edit"; onboard the sending domain under Email Service → Email Sending first (DKIM/SPF are auto-provisioned; add a DMARC record yourself). Mind the account's daily sending quota (dynamic, grows with reputation — throttle campaigns in listmonk to stay under it) and the plan's included monthly volume. Cloudflare has no bounce webhooks — its suppression list is automatic on their side; optionally sync failures back into listmonk via a cron polling the `emailSendingAdaptive` GraphQL dataset into listmonk's `POST /webhooks/bounce`. Enable listmonk's public subscription page so the form's plain POST to `/subscription/form` works. listmonk has no built-in RSS-to-email; a small scheduled job calling its campaign API with new feed items covers that. The privacy policy's "Email updates" section describes this self-hosted setup — revise it if you switch providers.
